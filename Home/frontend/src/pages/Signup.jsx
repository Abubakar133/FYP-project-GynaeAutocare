...
const SignUp = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    password: "",
    role: "patient",
    gender: "female", // Default gender for patient
    photo: selectedFile,
  });
  ...
  useEffect(() => {
    // Password validation check whenever password field changes
    const isPasswordValid = /^(?=.*[A-Z]).{8,}$/.test(formData.password);
              ...
              <div className="mb-5">
                <input
                  ...
                  className={`w-full pr-4 py-3 border-b border-solid ${
                    !passwordValid ? 'border-red-500' : 'border-[#0066ff61]'
                  } focus:outline-none focus:border-b-[#0067FF] text-[16px] leading-7 text-headingColor placeholder:text-textColor`}
                  required
                />
                {!passwordValid && (
                  <p className="text-red-500 text-sm mt-1">Password must be at least 8 characters long and contain at least one uppercase letter.</p>
                )}
              </div>
