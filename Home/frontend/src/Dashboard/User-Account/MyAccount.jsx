...
const MyAccount = () => {
  ...
  useEffect(() => {
    ...
    if (isHomePage) {
      ...
      localStorage.setItem("isFirstTimeVisit10", false);
    } else {
      // If it's not the home page, treat it as the first visit
      const isFirstTimeVisit = JSON.parse(localStorage.getItem("isFirstTimeVisit10"));
  
      if (!isFirstTimeVisit) {
        // Execute actions for the first visit
        redirectToDashboard2();
        localStorage.setItem("isFirstTimeVisit10", true);
      } else {
        localStorage.setItem("isFirstTimeVisit10", false);
